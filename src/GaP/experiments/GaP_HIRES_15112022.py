...
#
# $ python -m experiments.GaP_HIRES_15112022
#
...
path = '/scratch/SCS/eim/SMEAHEIA'
simcase = 'GEN_NOLGR_PH2'

# TODO(hzh): add input/output folders
data_folder = 'data/smeaheia'
simcase = os.path.join(data_folder, simcase)
...
# output file name
out_fname = os.path.join(data_folder, LGR_NAME+'.grdecl')
...
main_grd_dz_in_OB = make_stat_3d_main('DZ',0)[main_grd_i-1,main_grd_j-1,main_grd_min_k-1]       # 500.000 ft 

no_of_layers_in_OB = 10   # number of layer in overburden 
ref_depth = make_stat_3d_main('DEPTH',0)[main_grd_i-1,main_grd_j-1,main_grd_min_k-1] # reference depth where LGR (legacy well) starts
...
surf_Casing_ID = 0.3397           # 13-3/8 inch casing 

surf_Casing_strt_depth = 312
surf_Casing_end_depth = 686

surf_oph_strt_depth = 312
surf_oph_end_depth = 697

surf_Casing_strt_depth_cement = 312
surf_Casing_end_depth_cement = 686

################# Production Casing Design ########### 
prod_Casing_ID = 0.244             # 9-5/8 inch casing 

prod_Casing_strt_depth = 599
prod_Casing_end_depth = 1114

prod_oph_strt_depth = 599
prod_oph_end_depth = 1622

prod_Casing_strt_depth_cement = 683
prod_Casing_end_depth_cement = 1114

################# Barrier Desing #####################
has_barrier = 'yes'   # if the legacy well has barrier: 'yes', otherwise 'no'
barrier_ID = 0.3397
barrier_strt_depth = 352
barrier_end_depth = 560 
barrier_perm = 0.5

has_barrier = 'yes'   # if the legacy well has barrier: 'yes', otherwise 'no'
barrier_ID_2 = 0.244
barrier_strt_depth_2 = 1020
barrier_end_depth_2 = 1046 
barrier_perm_2 = 100
...
cond_bond = cond_Casing_ID - 2*surf_Casing_ID           # TODO(hzh): why multiply 2?
...
min_grd_size = 0.05                 # TODO(hzh): hard-coded
...
no_latral_grd_surf = no_grd_surf_case + no_grd_surf_bond*2       # TODO(hzh): why multiply 2?
...
LGR_sizes_xy = [(main_grd_dx- sum (LGR_size_no_outer))/2]+ LGR_size_no_outer + [(main_grd_dx- sum (LGR_size_no_outer))/2]  # TODO(hzh): add two more values
...
LGR_intvl [0] = LGR_intvl[0]+ ref_depth          # TODO(hzh): add ref_depth, necessary?
LGR_depths = (np.cumsum(LGR_intvl))

main_DZ[0] = ref_depth + main_DZ[0]   # TODO(hzh): not used, but what's for?
...
if has_barrier == 'yes':
    ...
    barrier_builder (barrier_ID_2, barrier_strt_depth_2,barrier_end_depth_2,barrier_perm_2) 
...
barrier_plotter(barrier_ID_2,  barrier_strt_depth_2, barrier_end_depth_2 ) 
