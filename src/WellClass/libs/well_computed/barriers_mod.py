...
def compute_barriers_diam(barriers: dict, borehole: dict) -> dict:
    ...
    for idx, row in barriers_df.iterrows():
        ...
        barrier_topD = borehole_df[check_top_int_top & check_bottom_int_top]["diameter_m"].iloc[0]
        barrier_bottomD = borehole_df[check_top_int_bottom & check_bottom_int_bottom]["diameter_m"].iloc[0]

        if barrier_topD == barrier_bottomD:
            barrier_name = f"{barrier:s}_{barr_idx:d}"
            ...
            barrier_query = borehole_df.query("diameter_m <= @barrier_topD and diameter_m >= @barrier_bottomD")
            for idx, sub_section in barrier_query.iterrows():
                ...
                barriers_fmt[barrier_name]["diameter_m"] = sub_section["diameter_m"]
                barriers_fmt[barrier_name]["barrier_perm"] = barrier_perm
                barr_idx += 1
