...
from .well_raw_validation import valid_casings, valid_drilling
...
class WellRaw:
    ...
    def _process_casings(self):
        if not self.inventory["casings"]:
            ...
            self.casings = [
                {
                    "top_rkb": _sf_depth_msl + _well_rkb,
    ...
    def _process_geology(self):
        if self.inventory["geology"]:
            ...
            synthetic_base = geology_df["top_rkb"] - geology_df["top_rkb"].diff(periods=-1)

            geology_df.loc[geology_df["base_rkb"].isna(), "base_rkb"] = synthetic_base[geology_df["base_rkb"].isna()]

            geology_df["base_msl"] = geology_df["base_rkb"] - self.header["well_rkb"]
