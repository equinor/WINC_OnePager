...
def plot_pressure(
    ...
    if plot_HSP:
        ax.plot(
            my_pressure.init_curves["hydrostatic_pressure"],
            my_pressure.init_curves["depth"],
    ...
    if len(scenarios_summary) > 0:
        ...
        for scenario_name, scenario in scenarios_summary.iterrows():
            ...
            if plot_fluid_contact:
                ax.scatter(sc_p_fluid_contact, sc_z_fluid_contact, color="blue", label="fluid contact")

            # Plot (p_resrv, z_resrv) points if option is turned on

            if plot_resrv:
                ax.scatter(sc_p_resrv, sc_z_resrv, color="green", label="reservoir pressure")
