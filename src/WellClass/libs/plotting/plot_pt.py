...
def plot_pt(
    ...
    if len(scenarios_summary) > 0:
        for scenario_name, scenario in scenarios_summary.iterrows():
            ...
            # plot co2 gradient
            ax.plot(
                sc_curves["temperature"],
                sc_curves["fluid_pressure"],
                label=f"{fluid}{sc_name}",
                color="firebrick",
                lw=0.75,
            ...
            # plot water gradient
            ax.plot(
                sc_curves["temperature"], sc_curves["brine_pressure"], label=f"brine {sc_name}", color="steelblue", lw=0.75, ls=next(line_style_cycle)
            )
            ...
            xmax = max(xmax, max_temp)

            ymax = max(ymax, base_msl)

            if plot_fluid_contact:
                ax.scatter(sc_t_fluid_contact, sc_p_fluid_contact, color="blue", label="fluid contact")

            if plot_resrv:
                ax.scatter(sc_t_resrv, sc_p_resrv, color="green", label="reservoir pressure")
            ...
            if plot_delta_p and sc_delta_p != 0:
                ...
                ax.vlines(
                    ...
                    color="black",
                    linestyle=":",
                    label=f"$\\Delta$P = {scenario['p_delta']:.0f} bar",
                )
