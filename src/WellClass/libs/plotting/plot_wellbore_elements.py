...
def geology_plotter(axis, df_geol, w_header, geol_bool, annot_bool, width, x_txt_pos, txt_size):
    if geol_bool:
        ...
        for idx, row in df_geol.iterrows():
            top_msl = row["top_msl"]
            base_msl = row["base_msl"]

            axis.hlines(y=top_msl, xmin=-width, xmax=width, zorder=-4, lw=0.25, color="k")
            axis.hlines(y=base_msl, xmin=-width, xmax=width, zorder=-4, lw=0.25, color="k")
            axis.axhspan(top_msl, base_msl, color="tan", alpha=0.5, zorder=-20)
