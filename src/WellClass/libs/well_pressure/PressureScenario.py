...
class PressureScenario:
    ...
    def _handle_from_reservoir(self) -> np.ndarray:
        ...
        if ip_params[["p_fluid_contact", "p_resrv", "p_delta"]].isna().all() and (
            "z_fluid_contact" in ip_params_with_value.index or "z_resrv" in ip_params_with_value.index
        ):
            ...
            "z_fluid_contact" in ip_params_with_value.index
            and "p_fluid_contact" in ip_params_with_value.index
            and "p_delta" not in ip_params_with_value.index
        ):
            ...
            "z_fluid_contact" in ip_params_with_value.index
            and "p_fluid_contact" in ip_params_with_value.index
            and "p_delta" in ip_params_with_value.index
        ):
            # Both z_fluid_contact and p_fluid_contact are provided
        ...
        fluid_pressure_profile = self._compute_fluid_pressure_curve(reference_depth=ref_z, reference_pressure=ref_p, fluid_key=self.fluid_type)
    ...
    def _handle_from_MSAD(self) -> np.ndarray:
        ...
        fluid_pressure_curve = self._compute_fluid_pressure_curve(
            reference_depth=self.z_MSAD, reference_pressure=self.p_MSAD, fluid_key=self.fluid_type
        )
        ...
        if self.z_fluid_contact is not None and self.z_MSAD > self.z_fluid_contact:
            ...
            self.p_delta = 0
            self.z_fluid_contact, self.p_fluid_contact = compute_intersection(
                self.init_curves["depth"].values, fluid_pressure_curve, self.init_curves["hydrostatic_pressure"].values
            )
