...
def _get_barrier_p_and_rho(pressure_scenarios: dict, pressure_CO2: dict, barrier_props: dict) -> pd.DataFrame:
    ...
    depth = pressure_CO2[
        ("init", "depth_msl")
    ]  # Depth look up table used in the interpolation of pressure and densities in the pressure_CO2 dataframe.
...
def _get_barrier_leakage(barrier_perm: dict, barrier_p_rho: pd.DataFrame, barrier_props: dict) -> pd.DataFrame:
    ...
    """
    logger.debug("barrier_perm: %s", barrier_perm["kv"])
...
def leakage_proxy(
    rho_fluid_below_barrier: float,
    rho_brine_below_barrier: float,
    p_fluid_below_barrier: float,
    p_brine_above_barrier: float,
    permeability: float,
    barrier_props: dict,
) -> float:
    """
